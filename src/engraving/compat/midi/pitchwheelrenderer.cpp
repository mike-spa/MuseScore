                                                 ...
                                                 uint32_t channel) const noexcept
{
    ...
    for (auto rit = ranges.crbegin(); rit != ranges.crend(); ++rit) {
        ...
        muse::vector<PitchWheelFunction> functionsToProcess;
...
int32_t PitchWheelRenderer::findNextStartTick(const muse::vector<PitchWheelFunction>& functions) const noexcept
{
...
int32_t PitchWheelRenderer::calculatePitchBend(const muse::vector<PitchWheelFunction>& functions, int32_t tick) const noexcept
{
...
// 3           |--------|            handleEndTick();
// result      |-------------------|
void PitchWheelRenderer::generateRanges(const muse::vector<PitchWheelFunction>& functions, std::map<int, int, std::greater<> >& ranges)
{
