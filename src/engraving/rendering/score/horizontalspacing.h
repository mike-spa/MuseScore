...
private:
    ...
    static void spaceMeasureGroup(const muse::vector<Measure*>& measureGroup, HorizontalSpacingContext& ctx);
    static double getFirstSegmentXPos(Segment* segment, HorizontalSpacingContext& ctx);
    static muse::vector<SegmentPosition> spaceSegments(const muse::vector<Segment*>& segList, int startSegIdx, HorizontalSpacingContext& ctx);
    static bool ignoreSegmentForSpacing(const Segment* segment);
    static bool ignoreAllSegmentsForSpacing(const muse::vector<SegmentPosition>& segmentPositions);
    static void spaceAgainstPreviousSegments(Segment* segment, muse::vector<SegmentPosition>& prevSegPositions,
    ...
    static void checkLyricsAgainstRightMargin(muse::vector<SegmentPosition>& segPositions, const HorizontalSpacingContext& ctx);
    static double spaceLyricsAgainstBarlines(Segment* firstSeg, Segment* secondSeg, const HorizontalSpacingContext& ctx);
    static void checkLargeTimeSigAgainstRightMargin(muse::vector<SegmentPosition>& segPositions);
    static void moveRightAlignedSegments(muse::vector<SegmentPosition>& placedSegments, const HorizontalSpacingContext& ctx);
    ...
    static void enforceMinimumMeasureWidths(const muse::vector<Measure*> measureGroup);
    static double computeMinMeasureWidth(Measure* m);
    static void stretchMeasureToTargetWidth(Measure* m, double targetWidth);

    static void stretchSegmentsToWidth(muse::vector<Spring>& springs, double width);

    static void setPositionsAndWidths(const muse::vector<SegmentPosition>& segmentPositions);
